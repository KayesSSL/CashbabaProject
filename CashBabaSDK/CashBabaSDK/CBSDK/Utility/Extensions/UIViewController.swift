...
extension UIViewController {
    ...
    func presentAsPopUp(vc: UIViewController , alpha:CGFloat = 0.3){
        vc.modalPresentationStyle = .custom
        
        vc.view.isOpaque = false
        vc.view.backgroundColor = UIColor.black.withAlphaComponent(alpha)
        // VC.popoverPresentationController?.delegate = self
        vc.popoverPresentationController?.sourceView = vc.view
        vc.popoverPresentationController?.sourceRect = CGRect(origin:CGPoint(x: 0, y: self.view.frame.maxY), size:CGSize(width: self.view.bounds.width, height: self.view.bounds.width) )
    ...
    public func showToastError(message: String) {
        let toastContainer = UIView(frame: CGRect())
